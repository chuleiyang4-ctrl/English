(() => {
  ...
  const mount = (panel) => {
    ...
    panel.innerHTML = `
      <div class="math-ai-head"><div><small>AI TUTOR</small><strong>English AI</strong></div><button type="button" class="math-ai-close" aria-label="Close English AI">×</button></div>
      <div class="math-ai-conversation" aria-live="polite"><div class="math-ai-message assistant">Ask about the current page, request examples, or practise English step by step.</div></div>
      <form class="math-ai-form">
        ...
        <label class="sr-only" for="math-ai-input">Ask English AI</label>
        <textarea id="math-ai-input" maxlength="${MAX_MESSAGE_LENGTH}" rows="3" placeholder="Ask an English question…" required></textarea>
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        if (!response.ok) throw new Error(data.error || "English AI is temporarily unavailable.");
